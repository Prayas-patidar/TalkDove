...
private static String otp,username="", email="",confirmpassword="",password="",gender="",address="",firstname="",lastname="",ccode="";
		...
		if(!rs.next()){
			...
			if(getPassword().equals(getConfirmpassword())){
				...
				if(otp.equals(Checkotp.otporg)){
					...
					// creating
					// connection with
					  PreparedStatement ps1 =  (PreparedStatement) con.prepareStatement("insert into register(username,email,password,firstname,lastname,gender,dateofbirth,phone,address,profilepic) values(?,?,?,?,?,?,?,?,?,?)");  // Creating PreparedStatement object
					   System.out.println("yaha pr hu");
					    ...
					    DBCreatePostTable.CreateTablePost(CurrentUserInfo.getEmail()); // Creating post table of Users
					    DBCreateNotify.CreateNotifyTable(CurrentUserInfo.getEmail());
					    DBCreateTableFollower.CreateFollowerTable(CurrentUserInfo.getEmail());
					    DBCreateStoryTable.CreateTableStory(CurrentUserInfo.getEmail());
					    System.out.println("ye bhi char raha return proper h");
					    CurrentUserInfo.setEmail(getEmail());
					    CurrentUserInfo.setPassword(getPassword());
					    CurrentUserInfo.setUsername(getUsername());
		// post
